import React, { useState, useEffect, useMemo } from 'react';
    ...
    import { CreditCard, Search, CheckCircle, Trash2, Loader2, Filter, Send, MoreHorizontal } from 'lucide-react';
    ...
    import BaixarParcelaDialog from '@/components/contas-receber/BaixarParcelaDialog';
    import DisparoCobrancaDialog from '@/components/contas-receber/DisparoCobrancaDialog';
    import NovaContaReceberDialog from '@/components/contas-receber/NovaContaReceberDialog';
    ...
    const ContasReceber = () => {
      ...
      const [disparoDialog, setDisparoDialog] = useState({
        open: false,
        conta: null
      });
      ...
      const availableYears = useMemo(() => {
        const years = new Set(contasReceber.map(c => format(parseISO(c.data_vencimento), 'yyyy')));
        return Array.from(years).sort().reverse();
      }, [contasReceber]);

      useEffect(() => {
        let result = contasReceber.map(c => {
        ...
        if (filters.busca) {
          result = result.filter(c => c.descricao.toLowerCase().includes(filters.busca.toLowerCase()) || c.cliente_nome.toLowerCase().includes(filters.busca.toLowerCase()));
        }
        if (filters.cliente !== 'all') {
          result = result.filter(c => c.cliente_id === filters.cliente);
        }
      ...
      }, [contasReceber, filters]);

      const categoriasEntrada = useMemo(() => {
        return categorias.raw?.filter(c => c.tipo === 'entrada') || [];
      }, [categorias.raw]);

      const handleMarkAsPaid = async pagamentoInfo => {
        setLoading(true);
        try {
          await contasReceberService.marcarComoPaga(pagamentoInfo);
          toast({
            title: "Sucesso!",
            description: "Parcela marcada como paga e lançamento criado.",
      ...
      const handleCreateConta = async formData => {
        setLoading(true);
        try {
          await contasReceberService.createVendaParcelada(formData);
          toast({
            title: "Sucesso!",
            description: "Nova conta a receber criada.",
            className: "bg-emerald-500 text-white"
          });
          await refreshData();
        } catch (error) {
      ...
      const handleDeleteConfirm = async () => {
        ...
        setLoading(true);
        try {
          await contasReceberService.deleteConta(contaToDelete.id);
          toast({
            title: "Sucesso!",
            description: "Conta a receber removida."
          });
          await refreshData();
        } catch (error) {
          toast({
      ...
      const summary = useMemo(() => {
        ...
        const totalPago = filteredContas.filter(c => c.derivedStatus === 'pago').reduce((sum, c) => sum + Number(c.valor_parcela), 0);
        return {
          totalPendente,
          totalAtrasado,
          totalPago
                ...
                <div className="space-y-3 text-sm text-gray-300">
                  ...
                  <div className="flex justify-end gap-2 pt-2">
                    {conta.status === 'pendente' && (
                      <>
                        <Button size="sm" variant="outline" onClick={() => setDisparoDialog({ open: true, conta: conta })}>
                          <Send className="w-4 h-4 mr-2" /> Cobrar
                        </Button>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          {conta.status === 'pendente' && (
                            <>
                              <DropdownMenuItem onClick={() => setDisparoDialog({ open: true, conta: conta })}><Send className="mr-2 h-4 w-4" /> Enviar Cobrança</DropdownMenuItem>
                              <DropdownMenuItem onClick={() => setPagamentoDialog({ open: true, conta: conta })}><CheckCircle className="mr-2 h-4 w-4" /> Baixar Parcela</DropdownMenuItem>
                            </>
                          )}
                          <DropdownMenuItem onClick={() => handleDeleteRequest(conta)} className="text-red-500"><Trash2 className="mr-2 h-4 w-4" /> Deletar</DropdownMenuItem>
                        </DropdownMenuContent>
                      </DropdownMenu>
                    </TableCell>
                  </TableRow>
      ...
      const FilterContent = () => (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4 items-end">
          ...
          <div>
              ...
              <Select value={filters.cliente} onValueChange={v => handleFilterChange('cliente', v)}>
          ...
          <div>
              ...
              <Select value={filters.categoria} onValueChange={v => handleFilterChange('categoria', v)}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent><SelectItem value="all">Todas</SelectItem>{categoriasEntrada.map(c => <SelectItem key={c.id} value={c.id}>{c.nome}</SelectItem>)}</SelectContent>
              </Select>
          </div>
          <div>
        ...
        <div className="space-y-6">
          <motion.div initial={{
            ...
            <div className="flex items-center space-x-3">
              <CreditCard className="w-8 h-8 sm:w-10 sm:h-10 text-cyan-400" />
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-gradient">Contas a Receber</h1>
                <p className="text-sm text-gray-400">Gerencie suas vendas parceladas e recebimentos futuros.</p>
              </div>
            </div>
            <NovaContaReceberDialog onConfirm={handleCreateConta} />
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card className="glass-effect"><CardContent className="p-4"><p className="text-sm text-gray-400">Total Recebido (Filtro)</p><p className="text-2xl font-bold text-emerald-400">{formatCurrency(summary.totalPago)}</p></CardContent></Card>
          </div>
          ...
          <Card className="glass-effect">
            ...
            <CardContent>
              {loading ? <div className="flex items-center justify-center h-64"><Loader2 className="w-8 h-8 animate-spin text-cyan-400" /></div> :
                !loading && filteredContas.length === 0 ? <div className="text-center text-gray-400 py-8">Nenhuma conta a receber encontrada.</div> :
                (isMobile ? <MobileView /> : <DesktopView />)
              }
            </CardContent>
          </Card>

          <BaixarParcelaDialog open={pagamentoDialog.open} onOpenChange={isOpen => setPagamentoDialog({ ...pagamentoDialog, open: isOpen })} conta={pagamentoDialog.conta} onConfirm={handleMarkAsPaid} />

          <DisparoCobrancaDialog open={disparoDialog.open} onOpenChange={isOpen => setDisparoDialog({ ...disparoDialog, open: isOpen })} conta={disparoDialog.conta} />

          <ConfirmationDialog open={confirmDeleteOpen} onOpenChange={setConfirmDeleteOpen} onConfirm={handleDeleteConfirm} title="Confirmar Exclusão" description="Tem certeza que deseja remover esta conta a receber? Esta ação não pode ser desfeita." />
        </div>
      );
    };
